...
function createMockReq(url: string, options?: { method?: string; body?: any }): IncomingMessage {
  ...
  req.headers = { host: "localhost:3008" };
...
describe("Notifications Service", () => {
  ...
  it("creates a notification", async () => {
    const { res } = await makeRequest("/send", {
      method: "POST",
      body: { userId: "user-1", title: "Test Notif", message: "Hello World", type: "info" },
    });
    expect(res._body.notification).toBeDefined();
    expect(res._body.notification.title).toBe("Test Notif");
    expect(res._body.notification.user_id).toBe("user-1");
    expect(res._body.notification.read).toBe(false);
  });

  it("marks notification as read", async () => {
    const createRes = await makeRequest("/send", {
      method: "POST",
      body: { userId: "user-2", title: "Read Me", message: "Please" },
    });
    const notifId = createRes.res._body.notification.id;
    const { res } = await makeRequest(`/notifications/${notifId}/read`, { method: "POST" });
    expect(res._body.notification.read).toBe(true);
  });

  it("lists notifications by userId", async () => {
    await makeRequest("/send", { method: "POST", body: { userId: "user-3", title: "A", message: "msg A" } });
    await makeRequest("/send", { method: "POST", body: { userId: "user-3", title: "B", message: "msg B" } });
    await makeRequest("/send", { method: "POST", body: { userId: "user-4", title: "C", message: "msg C" } });
    const { res } = await makeRequest("/notifications?userId=user-3");
    expect(res._body.notifications.length).toBe(2);
    expect(res._body.unread).toBe(2);
  });
});
