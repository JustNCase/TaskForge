...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('estimates')
    .select('*, clients(name), estimate_items(*)')
    .eq('user_id', user.id)
...
export async function POST(request: Request) {
  ...
  const { client_id, job_id, title, description, subtotal, tax_rate, tax_amount, total, notes, valid_until, items } = body
...
export async function PUT(request: Request) {
  ...
  if (!id) return NextResponse.json({ error: 'Estimate ID is required' }, { status: 400 })
