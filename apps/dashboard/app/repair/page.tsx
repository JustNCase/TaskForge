...
export default function RepairPage() {
  ...
  useEffect(() => {
    if (!user) return
    fetch('/api/tasks?category=repair&limit=100')
      .then(r => r.json())
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Link href="/tasks" className="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-sm">
          New Task
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center">
            <div className="p-3 bg-orange-100 dark:bg-orange-900 rounded-lg"><span className="text-2xl">🔧</span></div>
            <div className="ml-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 border border-gray-200 dark:border-gray-700">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Repair Tasks</h2>
        {isLoading ? <LoadingSkeleton rows={1} /> : tasks.length === 0 ? (
          <p className="text-center py-8 text-gray-500 dark:text-gray-400">No repair tasks yet.</p>
        ) : (
